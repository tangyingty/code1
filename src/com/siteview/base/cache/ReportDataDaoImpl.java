...
public class ReportDataDaoImpl implements ReportDataDao {
	...
	public void inserts(ErrorLogsBean data) throws Exception 
	{
		IniFile iniLog = DictionaryFactory.getErrorCompareReportLog();
		iniLog.createSection(data.getName());
//		iniLog.createSection("ccccc");
		iniLog.setKeyValue(data.getName(), "name", data.getName());
		iniLog.setKeyValue(data.getName(), "time",  data.getTime());
		iniLog.setKeyValue(data.getName(), "title", data.getTitle());
		iniLog.setKeyValue(data.getName(), "data", data.getData());
		iniLog.setKeyValue(data.getName(), "type", data.getType());
		iniLog.setKeyValue(data.getName(), "username", data.getUsername());
		iniLog.setKeyValue(data.getName(), "result", data.getResult());
		iniLog.setKeyValue(data.getName(), "target", data.getTarget());		
		iniLog.saveChange();
	}
	...
	public List<ErrorLogsBean> getValue() throws Exception 
	{
		...
		List<String> lstSections = iniLog.getSectionList();
		List<ErrorLogsBean> retlist = new ArrayList<ErrorLogsBean>(); 
		for(String strSection : lstSections)
		{
			 ErrorLogsBean data = new ErrorLogsBean();
			 data.setName(iniLog.getValue(strSection, "name"));
			 data.setData(iniLog.getValue(strSection, "data"));
			 data.setTime(iniLog.getValue(strSection, "time"));
			 data.setTitle(iniLog.getValue(strSection, "title"));
			 data.setType(iniLog.getValue(strSection, "type"));
			 data.setUsername(iniLog.getValue(strSection, "username"));
	         retlist.add(data);
		}
	...
	public void deleteValue(ErrorLogsBean errorLogsBean) throws Exception 
	{
		...
		String strDelSection = "";
		
		List<String> lstSections = iniLog.getSectionList();
		for(String strSection : lstSections)
		{			
			if(iniLog.getValue(strSection, "data").equals(errorLogsBean.getData()) && iniLog.getValue(strSection, "title").equals(errorLogsBean.getTitle())
					&& iniLog.getValue(strSection, "type").equals(errorLogsBean.getType()))
			{
				strDelSection = strSection;
				break;
			}
		}
		
		iniLog.deleteSection(strDelSection);	
	...
	public String selectValue(ErrorLogsBean errorLogsBean) throws Exception 
	{
		...
		{			
			if(iniLog.getValue(strSection, "data").equals(errorLogsBean.getData()) && iniLog.getValue(strSection, "title").equals(errorLogsBean.getTitle())
					&& iniLog.getValue(strSection, "type").equals(errorLogsBean.getType()))
			{
				return iniLog.getValue(strSection, "target");
			}
		}
