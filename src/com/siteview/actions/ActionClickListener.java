...
//				}
		...
		} else if (action.equals("delgroup"))
		{
			try
			{
				if("二层交换机,三层交换机,路由器,防火墙,服务器,PC设备,其他设备".contains(node.getName())){
					Message.showInfo(Labels.getLabel("DeviceGroupAssociatedWithNNMNotAllowedDelete"));
					return;
				}
				int r = Message.showQuestion(Labels.getLabel("ConfirmDeleteGroup:") + node.getName());
				...
				if (r == Messagebox.OK)
				{
					...
					if (delsuccess)
					{
						...
						if (parentnode == null)
						{
							minfo = Labels.getLabel("DeleteGroup:") + node.getName() + "(" + node.getSvId() + ")  ";
						} else
						{
							minfo = Labels.getLabel("DeleteGroup:") + node.getName() + "(" + node.getSvId() + ") parent is " + parentnode.getName() + "(" + parentnode.getSvId() + ")";
						}
		...
		} else if (action.equals("deldevice"))
		{
			try
			{
				int r = Message.showQuestion(Labels.getLabel("ConfirmDeleteEquipment:") + node.getName());
				Boolean delsuccess = false;
				if (r == Messagebox.OK)
				{
					Session session = Executions.getCurrent().getDesktop().getSession();
					...
					if (delsuccess)
					{
						...
						if (parentnode == null)
						{
							minfo = Labels.getLabel("RemoveDevice:") + node.getName() + "(" + node.getSvId() + ")";
						} else
						{
							minfo = Labels.getLabel("RemoveDevice:") + node.getName() + "(" + node.getSvId() + ") parent is " + parentnode.getName() + "(" + parentnode.getSvId() + ")";
						}
		...
		} else if (action.equals("delmonitor"))
		{
			try
			{
				int r = Message.showQuestion(Labels.getLabel("ConfirmDeleteMonitor:") + node.getName());
				Boolean delsuccess = false;
				if (r == Messagebox.OK)
				{
					Session session = Executions.getCurrent().getDesktop().getSession();
		...
		} else if (action.equals("startup"))
		{
			try
			{
				...
				if("group".equals(node.getType())){
					r = Message.showQuestion(Labels.getLabel("WhetherStartGroup:") + node.getName());
				}else if("entity".equals(node.getType())){
					r = Message.showQuestion(Labels.getLabel("WhetherStartEquipment:") + node.getName());
				}else if("monitor".equals(node.getType())){
					r = Message.showQuestion(Labels.getLabel("WhetherStartMonitor:") + node.getName());
				}
					...
					if (node.getType().equals(INode.GROUP))
					{
						...
						try
						{
							if(parentnode.getStatus().equals(INode.DISABLE)){
								Message.showInfo(Labels.getLabel("ParentNodeDisabled"));
								return;
							}
							...
							if (startsuccess)
							{
								...
								if (parentnode == null)
								{
									minfo = Labels.getLabel("StartGroup:") + node.getName() + "(" + node.getSvId() + ")";
								} else
								{
									minfo = Labels.getLabel("StartGroup:") + node.getName() + "(" + node.getSvId() + ") parent is " + parentnode.getName() + "(" + parentnode.getSvId() + ")";
								}
					...
					} else if (node.getType().equals(INode.ENTITY))
					{
						...
						try
						{
							...
							if (startsuccess)
							{
								...
								if (parentnode == null)
								{
									minfo = Labels.getLabel("BootDevice:") + node.getName() + "(" + node.getSvId() + ")";
								} else
								{
									minfo = Labels.getLabel("BootDevice:") + node.getName() + "(" + node.getSvId() + ") parent is " + parentnode.getName() + "(" + parentnode.getSvId() + ")";
								}
					...
					} else if (node.getType().equals(INode.MONITOR))
					{
						...
						try
						{
							...
							if (startsuccess)
							{
								...
								if (parentnode == null)
								{
									minfo = Labels.getLabel("StartMonitor:") + node.getName() + "(" + node.getSvId() + ")";
								} else
								{
									minfo = Labels.getLabel("StartMonitor:") + node.getName() + "(" + node.getSvId() + ") parent is " + parentnode.getName() + "(" + parentnode.getSvId() + ")";
								}
		...
		} else if (action.equals("pastedevice"))
		{
			...
			try
			{
				...
				if (pasternode == null)
				{
					Message.showInfo(Labels.getLabel("PCopyEquipment"));
					return;
				}
				...
				int r = Message.showQuestion(Labels.getLabel("WhetherStickingDevice:") + pasternode.getName());
				...
				if (entityid != null && !entityid.isEmpty())
				{
					...
					minfo = Labels.getLabel("StickingDevice:") + inode.getName() + "(" + entityid + ") to " + node.getName() + "(" + node.getSvId() + ")";
					
					AppendOperateLog.addOneLog(loginname, minfo, OpTypeId.paste, OpObjectId.entity);
				}
				Message.showInfo(Labels.getLabel("PastingDevice"));
		...
		} else if (action.equals("pastemonitor"))
		{
			...
			try
			{
				...
				if (pasternode == null)
				{
					Message.showInfo(Labels.getLabel("PCopyMonitor"));
					return;
				}
				...
				int r = Message.showQuestion(Labels.getLabel("WhetherPasteMonitor:") + pasternode.getName());
				if (r == Messagebox.CANCEL)
				{
					return;
				}
				...
				if (monitorid != null && !monitorid.isEmpty())
				{
					...
					minfo = Labels.getLabel("PasteMonitor:") + inode.getName() + "(" + monitorid + ") to " + node.getName() + "(" + node.getSvId() + ")";
					
					AppendOperateLog.addOneLog(loginname, minfo, OpTypeId.paste, OpObjectId.monitor);
				}
				Message.showInfo(Labels.getLabel("PasteMonitorSuccess"));
			...
			} catch (Exception ex)
			{
				...
				{
					smessage = Labels.getLabel("MonitorNotBelongToTypeEquipment");
				}
				if (smessage.contains("Less permission"))
				{
					smessage = Labels.getLabel("MonitorPointsNotEnough");
		...
		} else if (action.equals("batchDel"))
		{
			try
			{
				...
				{
					Message.showInfo(Labels.getLabel("NoSelectedItems"));
					return;
				}
				int r = Message.showQuestion(Labels.getLabel("SureDeleteSelectedItems"));
				if (r == Messagebox.OK)
				{
					...
					if (success)
					{
						...
						for (INode node2 : ids)
						{
							minfo = Labels.getLabel("Delete:") + node2.getName() + "(" + node2.getSvId() + ") parent is " + node.getName() + "(" + node.getSvId() + ")";
		...
		} else if (action.equals("batchStartup"))
		{
			try
			{
				...
				// return;
				// }
				int r = Message.showQuestion(Labels.getLabel("MonitoringWhetherActivateSelectedItems"));
				if (r == Messagebox.OK)
				{
