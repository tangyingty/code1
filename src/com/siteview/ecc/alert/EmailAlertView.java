...
public class EmailAlertView extends AbstractWindow {
		...
		public void onEvent(Event event) throws Exception {
			try{
				if(view.getStopCount().getValue()<view.getUpdateCount().getValue()){
					Message.showInfo(Labels.getLabel("StoppingTimeCannotLessThanNumberUpgrade"));
					return;
				}
				//新的规则：邮件接收地址 和 值班报警列表 至少存在一个 就 符合要求
				if(getWatchSheet().getValue().equals(Labels.getLabel("Empty"))&&getAlertReceiver().getValue().equals(Labels.getLabel("Other")) && getOtherEmailAddress().getValue().isEmpty()){
					Message.showInfo(Labels.getLabel("AlarmReceiveEmailAddressNotEmpty"));
					return;
				}
				if(!getOtherEmailAddress().getValue().isEmpty() && !BaseTools.validateEmail(getOtherEmailAddress().getValue())){
					Message.showInfo(Labels.getLabel("OtherE-mailAddressnotCorrectFormat"));
					return;
				}
				...
				alertinformation.setWatchSheet(Labels.getLabel("Empty").equals(view.getWatchSheet().getValue().trim())?"":view.getWatchSheet().getValue());
	...
	public void onCreate(Event event) throws Exception{
		try{
			...
			getAlertReceiver().setValue(Labels.getLabel("Other"));
			if (CMD_ADD.equals(this.getVariable(COMMAND_VAR, true))){
				...
				getWatchSheet().setValue(alertinformation.getWatchSheet()==null || "".equals(alertinformation.getWatchSheet()) ? Labels.getLabel("Empty") : alertinformation.getWatchSheet());
