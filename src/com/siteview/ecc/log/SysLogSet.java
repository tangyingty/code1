...
//	private Button applyDelButton;
	...
	public void onApply(Event event) {
		...
		try {
			...
			String minfo = loginname + Labels.getLabel("In") + OpObjectId.syslog_set.name + Labels.getLabel("Conducting")+OpTypeId.edit.name+Labels.getLabel("EquipmentParameterSettingOperation");
	...
	public void onApply2(Event event) {
		...
		try {
			...
			View view = Toolkit.getToolkit().getSvdbView(Executions.getCurrent().getDesktop());
			String loginname = view.getLoginName();
			String minfo = loginname + Labels.getLabel("In") + OpObjectId.syslog_set.name + Labels.getLabel("Conducting")+OpTypeId.edit.name+Labels.getLabel("LevelParameterSettingOperation");
			AppendOperateLog.addOneLog(loginname, minfo, OpTypeId.edit, OpObjectId.syslog_set);
	...
	public void onApplyDate(Event event) throws Exception {
		try{
			...
			View view = Toolkit.getToolkit().getSvdbView(Executions.getCurrent().getDesktop());
			String loginname = view.getLoginName();
			String minfo = loginname + Labels.getLabel("In") + OpObjectId.syslog_set.name + Labels.getLabel("Conducting")+OpTypeId.edit.name+Labels.getLabel("SystemLogRetainOperation");
			AppendOperateLog.addOneLog(loginname, minfo, OpTypeId.edit, OpObjectId.syslog_set);
