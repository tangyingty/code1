...
public class TableDaoImpl extends GenericAutowireComposer {
	...
	public void createLogOperateObj(){
		...
		setValue(listitem,Labels.getLabel("SignIn"),			"0",	"cbiObj0",	false);
		setValue(listitem,Labels.getLabel("Group"),				"2",	"cbiObj1",	false);
		setValue(listitem,Labels.getLabel("Equipment"),			"3",	"cbiObj2",	false);
		setValue(listitem,Labels.getLabel("Monitor"),			"4",	"cbiObj3",	false);
		setValue(listitem,Labels.getLabel("CustomerName"),		"6",	"cbiObj4",	false);
		setValue(listitem,Labels.getLabel("MonitoringServerName"),	"7",	"cbiObj5",	false);
		if(view.isAdmin()){
			setValue(listitem,Labels.getLabel("MonitorBrowsing"),		"22",	"cbiObj6",	false);
			setValue(listitem,Labels.getLabel("MonitorSettings"),		"23",	"cbiObj7",	false);
			setValue(listitem,Labels.getLabel("TopologyView"),		"9",	"cbiObj8",	false);
			setValue(listitem,Labels.getLabel("AlarmRules"),		"8",	"cbiObj9",	false);
			setValue(listitem,Labels.getLabel("StatisticalReport"),		"17",	"cbiObj10",	false);
			setValue(listitem,Labels.getLabel("TopNReport"),		"16",	"cbiObj11",	false);
			setValue(listitem,Labels.getLabel("BasicSetting"),		"5",	"cbiObj12",	false);
			setValue(listitem,Labels.getLabel("MailSettings"),		"12",	"cbiObj13",	false);
			setValue(listitem,Labels.getLabel("EmailTemplate"),		"24",	"cbiObj14",	false);
			setValue(listitem,Labels.getLabel("MessageSettings"),		"15",	"cbiObj15",	false);
			setValue(listitem,Labels.getLabel("SMSTemplates"),		"25",	"cbiObj16",	false);
			setValue(listitem,Labels.getLabel("DutyTableConfiguration"),		"18",	"cbiObj17",	false);
			setValue(listitem,Labels.getLabel("UserManagement"),		"11",	"cbiObj18",	false);
			setValue(listitem,Labels.getLabel("AbsoluteTimeTaskPlan"),"14",	"cbiObj19",	false);
			setValue(listitem,Labels.getLabel("TimeSlotTaskPlan"),	"13",	"cbiObj20",	false);
			setValue(listitem,Labels.getLabel("RelativeTimeTaskPlan"),"20",	"cbiObj21",	false);
			setValue(listitem,Labels.getLabel("SysLogSettings"),		"10",	"cbiObj22",	false);
			setValue(listitem,Labels.getLabel("IPAddress"),			"28",	"cbiObj28",	false);
		}else
		{
	...
	public void onQueryByCondition(Event event) throws Exception {
		try{
			...
			if(calStart.compareTo(calEnd)>=0){
				Messagebox.show(Labels.getLabel("StartTimeLessThanEndTime"), "提示", Messagebox.OK, Messagebox.INFORMATION);
				return;
			}
