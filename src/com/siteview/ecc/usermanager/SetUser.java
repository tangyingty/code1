	...
	public void onPermite(Event event) throws Exception{
		try {
			...
			for(Object it:listbox_data.getSelectedItems()){
				...
				String minfo=loginname+" "+Labels.getLabel("In")+OpObjectId.user_manage.name+Labels.getLabel("Conducting")+OpTypeId.enable.name+Labels.getLabel("OperationAllowingUsersTo:")+userName;
		...
		try{
			...
			if(i==1){
				...
				for(Object it:listbox_data.getSelectedItems()){
					...
					String userName = ini.getM_fmap().get(section).get("UserName");
					String minfo=view.getLoginName()+" "+Labels.getLabel("In")+OpObjectId.user_manage.name+Labels.getLabel("Conducting")+OpTypeId.del.name+Labels.getLabel("OperationDeleteUserName:")+userName;			
		...
		try {
			...
			for(Object it:listbox_data.getSelectedItems()){
				...
				if ("0".equals(flag) == false) {
					...
					ini.saveChange();
					String minfo=loginname+" "+Labels.getLabel("In")+OpObjectId.user_manage.name+Labels.getLabel("Conducting")+OpTypeId.diable.name+Labels.getLabel("OperationBannedUsers:")+userName;
