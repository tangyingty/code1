...
public class UserListbox extends AbstractListbox {
	...
	public List<String> getListheader() {
		return new ArrayList<String>(Arrays.asList(new String[] { Labels.getLabel("UserName"),
				Labels.getLabel("LoginName"),Labels.getLabel("State"),Labels.getLabel("UserTypes"),Labels.getLabel("Editor"),Labels.getLabel("GrantAuthorization")}));
	}
		...
		try {
			...
			for(User user:userList)
			{
				...
				for(String head : listhead){
					if(head.equals(Labels.getLabel("UserName"))){
					...
					if(head.equals(Labels.getLabel("LoginName"))){
					...
					if(head.equals(Labels.getLabel("State"))){						
						...
						if("1".equals(user.getNIsUse())){
							cell = new Listcell(Labels.getLabel("Enable"));//状态
							cell.setImage("/main/images/button/ico/enable_bt.gif");
						}else{
							cell = new Listcell(Labels.getLabel("Disable"));//状态
							cell.setImage("/main/images/button/ico/disable_bt.gif");
						}
						cell.setParent(item);
					}
					if(head.equals(Labels.getLabel("UserTypes"))){
						Listcell cell = null;//用户类型
						if("1".equals(user.getNAdmin())){
							cell = new Listcell(Labels.getLabel("AdministratorUser"));
						}else{
							cell = new Listcell(Labels.getLabel("OrdinaryUsers"));
						}
						cell.setParent(item);
					}
					...
					if(head.equals(Labels.getLabel("GrantAuthorization"))){						
