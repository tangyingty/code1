...
//			}			
			...
			for (String id : rd.getNodeidsArray())
			{
				...
				if(tempMap.keySet().contains(("(dstr)"+id)))
				{//example (dstr)1.9.7.1
...
//								System.out.println("@@flag:@@:"+flag1+"@@flag2:"+flag2);
								...
								{
									if(c ==1&&rdlist.size()>0)
									{//目的防止....... error erorr error|结束的情况
										TendencyCheckDataBean copyBean = rdlist.get(rdlist.size()-1);
//										copyBean.setEnd_when(tempTimeStr);
										copyBean.setStart_when(tempTimeStr);
										
										if(checkTime(copyBean)==false)
										{//开始时间小于结束时间
