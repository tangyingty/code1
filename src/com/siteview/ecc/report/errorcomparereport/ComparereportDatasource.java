...
public class ComparereportDatasource implements JRDataSource {
	...
	public Object getFieldValue(JRField field) throws JRException {
		...
		if (listbean.get(index) instanceof InputStream) {
			...
			//cxy 2013/04/03
			if ("MonitorName".equals(fieldName)) {
//			if ("ReturnName".equals(fieldName)) {				
			...
			} else if ("when_start".equals(fieldName)) {
				value = ((TendencyCheckDataBean) listbean.get(index)).getStart_when();
			} else if ("when_end".equals(fieldName)) {
				value = ((TendencyCheckDataBean) listbean.get(index)).getEnd_when();
			} else if ("errorValue".equals(fieldName)) {
				value = ((TendencyCheckDataBean) listbean.get(index)).getErrorValue();
			}
