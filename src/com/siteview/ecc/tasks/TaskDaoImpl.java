...
public class TaskDaoImpl extends GenericForwardComposer {
	...
	public void onAddButton(Event event) throws SuspendNotAllowedException,
		...
		win.setTitle(Labels.getLabel("AddAbsoluteTimeTaskPlan"));
	...
	public void onAddButton1(Event event) throws SuspendNotAllowedException,
		...
		win.setTitle(Labels.getLabel("AddTimeSlotTaskPlan"));
		win.setAttribute("flag", "add");
		win.setAttribute("nameList", nameList);
		...
		for(String s:sectionlist)
		{
			...
			if(StringTaskType != null && StringTaskName != null)
			{
				if(!"".equals(StringTaskName.trim()) && !"".equals(StringTaskType.trim())){
					...
					if(Labels.getLabel("AbsoluteTimeTaskPlan").equals(StringTaskType)){

						map.put("1", StringTaskName);
					}else if(Labels.getLabel("TimeSlotTaskPlan").equals(StringTaskType)){
				...
				if(deleteflag){// 能够删除
					...
					try{
						...
						if (flag.equals("abPage")){
							...
							+ OpObjectId.absolute_task.name + Labels.getLabel("Conducting")
							+ OpTypeId.del.name + Labels.getLabel("OperationDeleteItem:") + name;
							AppendOperateLog.addOneLog(loginname, minfo, OpTypeId.del,
							OpObjectId.absolute_task);
						}else if(flag.equals("perPage")){
							String minfo = loginname + " " + Labels.getLabel("In")
							+ OpObjectId.time_task.name + Labels.getLabel("Conducting")
							+ OpTypeId.del.name + Labels.getLabel("OperationDeleteItem:") + name;
							AppendOperateLog.addOneLog(loginname, minfo, OpTypeId.del,
						...
						{
							String minfo = loginname + " " + Labels.getLabel("In")
							+ OpObjectId.relative_task.name + Labels.getLabel("Conducting")
							+ OpTypeId.del.name + Labels.getLabel("OperationDeleteItem:") + name;
							AppendOperateLog.addOneLog(loginname, minfo, OpTypeId.del,
