		...
		for(String s:sectionlist)
		{
			...
			if(StringTaskType != null && StringTaskName != null)
			{
				if(!"".equals(StringTaskName.trim()) && !"".equals(StringTaskType.trim())){
					...
					if(Labels.getLabel("TimeSlotTaskPlan").equals(StringTaskType)){
			...
			if(deleteflag){
				try{
					...
					perTaskList.remove(name);
						String minfo = loginname + " " + Labels.getLabel("In")+ OpObjectId.time_task.name + Labels.getLabel("Conducting")+ OpTypeId.del.name + Labels.getLabel("OperationDeleteItem:") + name;
