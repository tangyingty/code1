...
public class SetDataBase extends GenericAutowireComposer {
	...
	public void onInit() throws Exception{
		...
		if(ini.getSectionData("DataBaseConfig") == null){
			...
			ini.setKeyValue("DataBaseConfig", "PramDesc0", Labels.getLabel("PhoneNumber"));
			...
			ini.setKeyValue("DataBaseConfig", "PramDesc1", Labels.getLabel("ContentOfMessage"));
			ini.saveChange();
			ini.load();//重新加载
		}
		...
		for (int i = 0; i < count; i++) {
			...
			t2.setHeight("15px");
			t2.setWidth("80px");
			Combobox cb2 = myComboBox(Labels.getLabel("PhoneNumber"), Labels.getLabel("ContentOfMessage"), Labels.getLabel("SendTime"));
			cb2.setHeight("15px");
			cb2.setWidth("80px");
			// 取值
		...
		if (ini.getValue("DataBaseConfig", "InsertWay") != null)
			if (ini.getKeyList("DataBaseConfig").contains("PorcType")) {
				returnValue
						.setValue(Labels.getLabel("ParameterType:")
								+ ini.getValue("DataBaseConfig", "PorcType")
								+ Labels.getLabel("ParameterLength")
								+ ini.getValue("DataBaseConfig", "PorcLength")
								+ Labels.getLabel("ParameterDescription")
	...
	public void onDel(Event event){
		...
		try{
			...
			String minfo = loginname + " " + Labels.getLabel("In") + OpObjectId.sms_set.name
					+ Labels.getLabel("Conducting") + OpTypeId.del.name + Labels.getLabel("OperationDeletedInDatabaseConfigurationInformation");
	...
	public void onAdd(Event event)  throws Exception{
		...
		if (param.getChildren().size() >= 4)
			...
			String loginname = view.getLoginName();
			String minfo = loginname + " " + Labels.getLabel("In") + OpObjectId.sms_set.name
					+ Labels.getLabel("Conducting") + OpTypeId.add.name + Labels.getLabel("OperationAddConfigurationInformationDatabase");
		...
		try{
			...
			for (; it.hasNext(); i++) {
				...
				if (!(((Textbox) (((Listcell) l1.get(0)).getChildren()).get(0))
					...
					try{
						Messagebox.show(Labels.getLabel("ParameterNameCannotEmpty"));
					}catch(Exception e){}
					return;
				}
	...
	public void onMore()  {
		try{
			...
			win.setTitle(Labels.getLabel("EditReturnValue"));
