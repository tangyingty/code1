...
public class MessageTemplateSet extends GenericAutowireComposer {
	...
	public void onAdd(Event event) throws Exception{
		...
		try{
			...
			String minfo=loginname+" "+Labels.getLabel("In")+OpObjectId.message_template.name+Labels.getLabel("Conducting")+OpTypeId.add.name+Labels.getLabel("SMSTemplateOperationAddMessageTemplate:")+ templateTextbox.getValue();
	...
	public void onDel(Event event)  throws Exception{
		...
		try{
			...
			if(flag.contains("&")){	
				...
				View view = Toolkit.getToolkit().getSvdbView(event.getTarget().getDesktop());
				String loginname = view.getLoginName();
				String minfo=loginname+" "+Labels.getLabel("In")+OpObjectId.message_template.name+Labels.getLabel("Conducting")+OpTypeId.del.name+"短信模板操作，删除的短信模为： "+ templateTextbox.getValue();
